...
void readTerm(const string& inStr, bitset<n>& _Out)
{
    ...
    while (iScan != inStr.length())
    {
        string bitStr;
        bitStr += inStr[iScan++];
        while (inStr[iScan] != 'k' && inStr[iScan] != 'X'   && iScan != inStr.length())
            bitStr += inStr[iScan++];
...
int main(int argc, char* argv[])
{
	...
	fstream fs2;
	fs2.open("result/superpoly/search_log.txt", ios::app);
	fs2<< "decube: " << decube0<< "_" << decube1<< "_"<< decube2<<"_"<< decube3<< endl;
	fs2<< "Number of output 1: " << cnt_1 << endl;
	fs2<< "Balancedness : " << ((double)cnt_1) / (1 << 15) << endl<<endl;
	if ( (((double)cnt_1) / (1 << 15)) >= 0.49 &&   (((double)cnt_1) / (1 << 15)) <= 0.51  )  
	{
	fs2<< "May be valuable!"<<endl;
	}
	else{
	fs2<< "Useless!"<<endl;	
	}
    ...
    sprintf(tmpfilename, "res_%d_%d_%d_%d.txt",decube0,decube1,decube2,decube3);
    
    if ( (((double)cnt_1) / (1 << 15)) >= 0.49 &&   (((double)cnt_1) / (1 << 15)) <= 0.51  )  
	{
		
		path outputDir("tmp/may_valuable_superpolys/");
	    outputDir.append(tmpfilename);
	    cout << "Output superpoly to : "<<outputDir.string() << endl;
	
	    fs.open(outputDir.string(), ios::out);
	    for(auto & x : lineMap)
	        if (x.second % 2)
	        {
    	...
    	path outputDir("result/superpoly/useless_superpolys/");
	    outputDir.append(tmpfilename);
	    cout << "Output superpoly to : "<<outputDir.string() << endl;
