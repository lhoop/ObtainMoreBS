...
{
    string filePath = "tmp/LOG/log_" + getCurrentSystemTime( DATEFLAG ) + ".log"; 
...
{
    string filePath = "./tmp/STATE/state_" + to_string(rounds) + ".log";
    ofstream os(filePath.c_str(), ios_base::out | ios_base::app);
...
inline void loggerStream(stringstream& ss, int rounds)
{
    ...
    string filePath = "./tmp/TERM/term_" + to_string(rounds) +"_" + threadIdStr + ".log";
    ofstream os(filePath.c_str(), ios_base::out | ios_base::app);
